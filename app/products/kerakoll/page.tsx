...
export const metadata: Metadata = {
  title: 'Kerakoll : Build360.online',
  description: 'Kerakoll eco-sustainable tile adhesives, resin & mineral grouts, sealants, and waterproofing systems — source in bulk.',
};

const brand = brands.find((b) => b.id === 'kerakoll')!;
const brandProducts = products.filter((p) => p.brand === 'kerakoll');

export default function KerakollPage() {
  return (
    <>
                  ...
                  {product.specs && (
                    <div className="flex flex-wrap gap-2 mt-3">
                      {Object.entries(product.specs).map(([key, val]) => (
                        <span key={key} className="px-2 py-1 text-[9px] font-mono uppercase tracking-wider bg-[var(--color-primary-10)] text-[var(--color-primary)] rounded">
                          {val}
                        </span>
                      ))}
                    </div>
